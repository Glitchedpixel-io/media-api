...
from tools.artwork_backfill.backfill import COVER_KIND, find_cover, run
...
@pytest.fixture
def cover_kind_id(artwork_kind_ids: dict[str, int]) -> int:
    """The kind a found-on-disk cover is registered as -- `unknown` since #154, because
    this tool has no declaring client to ask."""
    return artwork_kind_ids[COVER_KIND]
...
class TestDryRun:
    ...
    def test_a_dry_run_writes_no_file_and_no_row(
        self, db_session, store, accessory_root, artwork_root, cover_kind_id, make_asset
    ):
        asset_id = make_asset()
        _place_cover(accessory_root, asset_id, JPEG)

        summary = run(db_session, store, accessory_root, cover_kind_id, dry_run=True)
    ...
    def test_a_dry_run_still_refuses_what_the_real_run_would(
        self, db_session, store, accessory_root, cover_kind_id, make_asset
    ):
...
class TestApply:
    ...
    def test_registers_a_cover_and_writes_the_file(
        ...
        summary = run(db_session, store, accessory_root, cover_kind_id, dry_run=False)

        assert summary.registered == 1
        rows = _artwork_for(db_session, asset_id)
        assert len(rows) == 1
        assert rows[0].artwork_kind == COVER_KIND
    ...
    def test_the_registered_artwork_is_primary(
        ...
        asset_id = make_asset()
        _place_cover(accessory_root, asset_id, JPEG)
        run(db_session, store, accessory_root, cover_kind_id, dry_run=False)
...
class TestIdempotency:
    ...
    def test_a_second_run_registers_nothing_new(
        ...
        first = run(db_session, store, accessory_root, cover_kind_id, dry_run=False)
        second = run(db_session, store, accessory_root, cover_kind_id, dry_run=False)
...
class TestResilience:
    ...
    def test_a_failed_insert_does_not_poison_the_rest_of_the_pass(
        self, db_session, store, accessory_root, cover_kind_id, make_asset, monkeypatch
    ):
...
class TestLimit:
    ...
    def test_limit_stops_the_pass_and_says_so(
        ...
        summary = run(db_session, store, accessory_root, cover_kind_id, dry_run=False, limit=2)
    ...
    def test_a_complete_pass_does_not_claim_it_was_truncated(
        ...
        summary = run(db_session, store, accessory_root, cover_kind_id, dry_run=False, limit=5)
    ...
    def test_a_limited_run_can_be_resumed(
        ...
        run(db_session, store, accessory_root, cover_kind_id, dry_run=False, limit=2)
        second = run(db_session, store, accessory_root, cover_kind_id, dry_run=False)
...
@pytest.mark.integration
class TestDeclaredKind:
    """This tool has no declaring client, so it declares nothing (#154).

    Everywhere else a caller says what the artwork is and the server checks the pixels
    do not contradict it. Here a file is found on disk and `cover.*` says nothing about
    what the image is -- asserting a kind anyway is what produced #138.
    """

    def test_the_default_kind_is_unknown(self):
        assert COVER_KIND == "unknown"

    def test_a_registered_cover_carries_the_unknown_kind(
        self, db_session, store, accessory_root, cover_kind_id, make_asset
    ):
        asset_id = make_asset()
        _place_cover(accessory_root, asset_id, JPEG)
        ...
        rows = _artwork_for(db_session, asset_id)
        assert [row.artwork_kind for row in rows] == ["unknown"]

    def test_a_landscape_cover_is_registered_rather_than_refused(
        self, db_session, store, accessory_root, cover_kind_id, make_asset
    ):
        """`unknown` carries no shape expectation, so the 16:9 covers that make up most
        of the store are registered rather than rejected by #153's check. Declaring
        `poster` here would refuse every one of them."""
        asset_id = make_asset()
        _place_cover(accessory_root, asset_id, _image_bytes(1920, 1080))
...
@pytest.mark.integration
class TestSkipsAssetsThatAlreadyHaveArtwork:
    """The pre-load asks "has artwork", not "has artwork of this kind" (#154).

    Scoped to one kind it missed an asset whose cover had since been reclassified --
    which #155 does to every row this tool wrote -- so the next pass would re-store the
    file and re-attempt the insert for all of them.
    """

    def test_an_asset_with_artwork_of_another_kind_is_skipped(
        self, db_session, store, accessory_root, artwork_kind_ids, cover_kind_id, make_asset
    ):
        asset_id = make_asset()
        _place_cover(accessory_root, asset_id, JPEG)
        run(db_session, store, accessory_root, artwork_kind_ids["thumbnail"], dry_run=False)
    ...
    def test_an_asset_with_no_artwork_is_still_visited(
        self, db_session, store, accessory_root, artwork_kind_ids, cover_kind_id, make_asset
    ):
        """The widened check must not skip the assets the pass exists for."""
        covered = make_asset()
        bare = make_asset()
        _place_cover(accessory_root, covered, JPEG)
        run(db_session, store, accessory_root, artwork_kind_ids["thumbnail"], dry_run=False)

        # bare's cover only appears after the first pass, so it is the one asset the
        # second pass should still have work to do for.
        _place_cover(accessory_root, bare, JPEG)
        second = run(db_session, store, accessory_root, cover_kind_id, dry_run=False)

        assert second.registered == 1
        assert _artwork_for(db_session, bare) != []
